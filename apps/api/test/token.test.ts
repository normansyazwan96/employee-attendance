...
describe("access tokens", () => {
  it("accepts a valid token", () => {
    const token = createAccessToken({ sub: "user-1", email: "user@example.com", role: "EMPLOYEE" });
    expect(verifyAccessToken(token)?.sub).toBe("user-1");
  });

  it("rejects a token with malformed claims", () => {
    const header = encode({ alg: "HS256", typ: "JWT" });
    const body = encode({ sub: "user-1", email: "not-an-email", role: "EMPLOYEE", exp: Math.floor(Date.now() / 1000) + 60 });
  ...
  it("rejects an expired token", () => {
    const header = encode({ alg: "HS256", typ: "JWT" });
    const body = encode({ sub: "user-1", email: "user@example.com", role: "EMPLOYEE", exp: Math.floor(Date.now() / 1000) - 1 });
    const signature = createHmac("sha256", process.env.JWT_SECRET).update(`${header}.${body}`).digest("base64url");
    expect(verifyAccessToken(`${header}.${body}.${signature}`)).toBeNull();
  });
});