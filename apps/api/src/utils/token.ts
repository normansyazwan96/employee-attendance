...
export type AuthTokenPayload = { sub: string; role: "EMPLOYEE" | "ADMIN" | "DEV_ADMIN"; email: string; exp: number };
const tokenPayloadSchema = z.object({ sub: z.string().min(1), email: z.string().email(), role: z.enum(["EMPLOYEE", "ADMIN", "DEV_ADMIN"]), exp: z.number().int().positive() });
const encode = (value: object): string => Buffer.from(JSON.stringify(value)).toString("base64url");
const signature = (input: string): string => createHmac("sha256", env.JWT_SECRET).update(input).digest("base64url");

export function createAccessToken(payload: Omit<AuthTokenPayload, "exp">): string {
  const header = encode({ alg: "HS256", typ: "JWT" });
  const body = encode({ ...payload, exp: Math.floor(Date.now() / 1000) + 60 * 60 * 8 });
