...
export async function login(request: Request, response: Response): Promise<void> {
  ...
  const token = createAccessToken({ sub: user.id, email: user.email, role: user.role, passwordVersion: user.passwordVersion });
  response.json({ token, user: { id: user.id, email: user.email, role: user.role, name: user.employee ? `${user.employee.firstName} ${user.employee.lastName}` : user.displayName, mustChangePassword: user.mustChangePassword } });
}
