...
const profileSchema = z.object({ email: z.string().email().transform((value) => value.toLowerCase()), displayName: z.string().trim().min(1).max(120).nullable(), firstName: z.string().trim().min(1).max(80).optional(), lastName: z.string().trim().min(1).max(80).optional(), currentPassword: z.string().min(1).optional(), newPassword: strongPassword.optional() });

function view(user: { id: string; email: string; role: string; displayName: string | null; mustChangePassword: boolean; employee: { firstName: string; lastName: string } | null }) { return { id: user.id, email: user.email, role: user.role, displayName: user.displayName, firstName: user.employee?.firstName ?? "", lastName: user.employee?.lastName ?? "", mustChangePassword: user.mustChangePassword }; }
...
export async function updateProfile(request: Request, response: Response): Promise<void> {
  ...
  const token = parsed.data.newPassword ? createAccessToken({ sub: user.id, email: user.email, role: user.role, passwordVersion: user.passwordVersion }) : undefined;
