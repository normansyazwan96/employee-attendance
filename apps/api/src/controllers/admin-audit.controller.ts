...
export async function getAdminAudit(_request: Request, response: Response): Promise<void> {
  ...
  const logs = await prisma.auditLog.findMany({ where: auth.role === Role.DEV_ADMIN ? undefined : { clientId: user?.clientId, actorUserId: auth.sub }, orderBy: { createdAt: "desc" }, take: 100 });
